...
//------------------FRAGMENT FOR DRAWING THE DEGREE SYMBOL (°)--------------------------------------
void GFX_DrawDegreeSymbol(int x, int y, uint8_t color)
{
    // Bitmap for the degree symbol (°)
	const uint8_t degree_symbol[] = {
	    0x0E,  // 01110 (Top part of the circle)
	    0x15,  // 10101
	    0x15,  // 10101
	    0x0E,  // 01110 (Bottom part of the circle)
	    0x00,  // 00000
	    0x00,  // 00000
	    0x00,  // 00000
	    0x00   // 00000
    ...
    // Drawing the degree symbol (8x8)
    for (int j = 0; j < 8; j++) {  // Iterate through bitmap rows
        for (int i = 0; i < 8; i++) {  // Iterate through bitmap columns
            if (degree_symbol[j] & (0x80 >> i)) {  // Check if the pixel is active
                GFX_DrawPixel(x + i, y + j, color);  // Draw the pixel on the display
            }
        }
    }
}
...
uint16_t GFX_GetTextWidth(char* str) {
    uint16_t width = 0;
    char znak;
    while ((znak = *str++)) {
        width += GFX_GetFontWidth(); // Add font width for each character
    }
    return width;
}

void DrawAngleWithDegree(uint16_t AngleY) {
    // Display the angle value
	char MessageAngle[32];
    sprintf(MessageAngle, "Angle: %u ", AngleY);
    uint16_t textWidth = GFX_GetTextWidth(MessageAngle); // Calculate text width
    GFX_DrawString(0, 30, MessageAngle, WHITE, 0);  // Draw text

    // Draw the degree symbol (°) right after the number
    GFX_DrawDegreeSymbol(0 + textWidth + 2, 30, WHITE);  // Shift the degree symbol
}
